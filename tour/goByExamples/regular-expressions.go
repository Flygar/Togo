...
func main() {
	// 这个测试一个字符串是否符合一个表达式。
	match, _ := regexp.MatchString("p([a-z]+)ch", "peach") // p([a-z]{2})ch
	fmt.Println(match)

	// 上面我们是直接使用字符串，但是对于一些其他的正则任务，你需要 `Compile` 一个优化的 `Regexp` 结构体。
	r, _ := regexp.Compile("p([a-z]{2})ch")
	...
	fmt.Println(r.Match([]byte("aaaapeachaaaa")))

	// 查找匹配字符串。首次匹配就返回
	fmt.Println(r.FindString("peeeeach punch"))
